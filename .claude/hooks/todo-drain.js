...
// Claude завершает ход, а в секции "## В РАБОТЕ" файла TODO.md остались задачи [ ] →
...
//  - задачи [~] (в работе) и [?] (ждут решения) не триггерят
//  - нет TODO.md → молчим
...
process.stdin.on("end", () => {
  ...
  // --- 1. Считаем невзятые задачи в секции "В РАБОТЕ"
  // Регекс устойчив к эмодзи в заголовке (## 🟢 В РАБОТЕ) и к пробелам.
  ...
  process.stderr.write(
    "STOP-ХУК (дренаж TODO): в секции «В РАБОТЕ» осталось задач: " + openTasks +
      " (автоцикл " + state.count + "/" + MAX_AUTO + "). Правило: закрыл задачу → бери следующую, не спрашивая «что дальше». " +
      "Действуй: 1) отчитайся ✅ по сделанному (коротко); 2) возьми ВЕРХНЮЮ [ ] из «В РАБОТЕ», пометь [~]; 3) выполни по правилам CLAUDE.md, по готовности пометь [x] и допиши 1–2 строки в ЖУРНАЛ (что+WHY). " +
      "Останавливаться можно только если: есть настоящий продуктовый вопрос к Заказчику (задай его с маркером ❓, задачу пометь [?] и перенеси в «ОЖИДАЕТ РЕШЕНИЯ ЗАКАЗЧИКА») или очередь пуста."
  );
  process.exit(2);
});
