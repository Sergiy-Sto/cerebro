...
ФОРМАТ ОПИСАНИЯ (description) — модели имеют тенденцию ужимать, тут это запрещено:

- **Объём: 800-1800 символов.** Минимум 800, цель ~1200-1500. Меньше = брак (поверхностно, следующие этапы получат пустоту).
- **Перечисления** оформляй маркированным списком через переносы строк (\\n). Каждый пункт = **название + минимум 2 содержательных предложения** (суть, контекст, специфика). Одна фраза = брак.
- **Минимум пунктов в перечислении:** 8, цель 10-14. Не выбирай "топ-3 для краткости" — раскрывай полностью.
- **Линейная мысль** (аналитический вывод) — развёрнутыми абзацами с конкретными примерами, цифрами, именами.
- **Self-check перед записью:** "У меня 8+ пунктов с 2+ предложениями каждый? Если нет — добавь до требования."
- **Анти-сжатие:** стремись к МАКСИМУМУ информации в рамках формата, не к минимуму. Эта карточка — единственный источник по теме для следующих этапов.
- Конкретика (цифры, примеры, имена) лучше общих формулировок.
...
ПРАВИЛА (читай внимательно — модели имеют тенденцию ужимать, тут это запрещено):

**1. Жёсткий минимум объёма.** Для категорий с перечислением (ТИПЫ, ФОРМАТЫ, КРАЙНИЕ СЛУЧАИ, СУЩЕСТВУЮЩИЕ РЕШЕНИЯ, ЗАМЕНИТЕЛИ, СМЕЖНЫЕ СУЩНОСТИ): **минимум 8 пунктов, цель 10-14**. Это не "если знаешь" — это база. Если в первой версии получилось 5, ты обязан подумать ещё и добавить до 8+.

**2. Раскрытие каждого пункта.** Каждый пункт = название + **минимум 2 содержательных предложения**: что это, контекст использования, специфика/нюансы. Одна фраза-определение = брак.

**3. Образец качества (ориентируйся на этот уровень детализации):**
---
Основные виды ремонта в реальном рынке:
• Косметический — обновление визуальных элементов без вмешательства в инженерные системы: покраска/обои, замена напольных покрытий, новые двери и плинтуса, иногда замена сантехники без труб. Делают раз в 5-10 лет в квартирах в нормальном состоянии. Срок 2-6 недель, бюджет 8-20% от капитального.
• Капитальный — демонтаж до бетона/кирпича, замена электрики, сантехники, вентиляции, выравнивание стен и полов, монтаж новой отделки. Обычно после покупки вторички в плохом состоянии или при перепланировке. Срок 3-6 месяцев, бюджет в 3-5× выше косметического.
---

**4. Self-check перед записью.** Прогони себя по чек-листу:
   - У меня 8+ пунктов? Если нет — добавь.
   - Каждый пункт раскрыт 2+ предложениями? Если нет — раскрой.
   - Не упустил ли я важных подвидов, нишевых вариантов, региональных особенностей? Если упустил — добавь.

**5. Анти-сжатие.** Стремись к **максимуму информации**, не к минимуму. Эта карточка — единственный источник по этой категории для 13 следующих этапов методологии. Скупо здесь = пусто там.

**6. Аналитические категории** (ИЗВЕСТНЫЕ ПРОБЛЕМЫ, ПРОБЕЛЫ, НЕОЧЕВИДНЫЕ ФАКТЫ) — развёрнутые абзацы с конкретными примерами и цифрами.

**7. Помечай предположения:** "предположительно...", "стоит проверить поиском..." — когда не уверен.

**8. Title формат:** "<КАТЕГОРИЯ>: <конкретное наблюдение/ключевая находка>".

Цель этапа: максимально насыщенная карта реальности. Не креатив, не реферат — энциклопедия по конкретной категории.
