...
- Объём: 600-1500 символов. Не реферат, но и не телеграммой. Поверхностное описание разрушает следующие этапы — там не из чего будет строить.
- Когда есть перечисление (типы, варианты, примеры, причины, признаки) — оформляй маркированным списком через переносы строк. Каждый пункт = **название + 2-3 предложения раскрытия**: суть, контекст, специфика, когда встречается. НЕ одна фраза.
- Когда мысль линейная (структурное наблюдение, аналитический вывод) — пиши развёрнутыми абзацами с конкретными примерами.
- ЗАПРЕТ на искусственное сокращение: если знаешь 10 примеров — перечисли все 10, не выбирай "топ-3 для краткости". Это карточка-фундамент, на ней строятся 13 следующих этапов.
- Конкретика всегда лучше общих формулировок.
...
- В категориях с перечислениями (ТИПЫ, ФОРМАТЫ, КРАЙНИЕ СЛУЧАИ, СУЩЕСТВУЮЩИЕ РЕШЕНИЯ, ЗАМЕНИТЕЛИ, СМЕЖНЫЕ СУЩНОСТИ) — формат: "вступительная фраза, объясняющая категорию:\\n• Название — 2-3 предложения раскрытия (что это, в каком контексте, какая специфика).\\n• ..."
- **Минимум 6-10 пунктов в каждой категории с перечислением.** Не сокращай ради лаконичности. Если знаешь 12 типов — перечисли все 12. Эта карточка — единственный источник по этой категории для всех последующих этапов методологии (Entity Mapping, Friction Map, Hypotheses и т.д.). Скупо здесь = пусто там.
- В категориях с аналитическим выводом (ИЗВЕСТНЫЕ ПРОБЛЕМЫ, ПРОБЕЛЫ В ЗНАНИЯХ, НЕОЧЕВИДНЫЕ ФАКТЫ) — развёрнутые абзацы с конкретными примерами и цифрами где можешь.
- Если что-то — твоё предположение, а не известный факт, помечай: "предположительно...", "стоит проверить поиском...".
- Title формат: "<КАТЕГОРИЯ>: <конкретное наблюдение/ключевая находка>".
- Это карта реальности, не креатив. Цель — максимально насыщенная картина того, что ЕСТЬ.
