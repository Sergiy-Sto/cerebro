...
echo "== think-deeper: mention-guard (упоминания не триггерят) =="
check "«подумай» в кавычках → тихо"     no  '{"prompt":"как это бывает со словом «подумай»"}'
check "подумай-граблей (дефис) → тихо"  no  '{"prompt":"без подумай-граблей будет лучше"}'
check "«слово подумай» (маркер) → тихо" no  '{"prompt":"слово подумай триггерит хук"}'
check "реальная команда → гейт"         yes '{"prompt":"подумай над этим как следует"}'
