...
skillcall() { printf '{"message":{"content":[{"type":"tool_use","name":"Skill","input":{"skill":"design:design-critique"}}]}}'; }
run() { rm -f .claude/visual-check-mobile-state.json .claude/visual-check-design-state.json; printf '{"transcript_path":"%s","stop_hook_active":false}' "$TMP" | node "$HOOK" >/dev/null 2>&1; echo $?; }
...
echo "== visual-check: визуал-claim + дизайн-нудж =="
{ user_block; printf '\n'; browse; printf '\n'; claim 'Создал страницу Аксессуары, готово.'; } > "$TMP"
massert "создал страницу + DOM-чек, без скрина → нудж(скрин)" 2
{ user_block; printf '\n'; claim 'Создал страницу, вёрстка готова.'; } > "$TMP"
massert "визуал-claim БЕЗ браузера → не нудж" 0
{ user_block; printf '\n'; browse; printf '\n'; ss_block; printf '\n'; claim 'Создал страницу, готово, 27px норма.'; } > "$TMP"
massert "создал страницу + скрин, critique НЕ вызван → дизайн-нудж" 2
{ user_block; printf '\n'; browse; printf '\n'; ss_block; printf '\n'; claim 'Создал страницу, готово. Оценил: воздух и баланс ок.'; } > "$TMP"
massert "создал страницу + скрин + лексика без скилла → дизайн-нудж (лексику не засчитываем)" 2
{ user_block; printf '\n'; browse; printf '\n'; skillcall; printf '\n'; ss_block; printf '\n'; claim 'Создал страницу, прогнал critique, готово.'; } > "$TMP"
massert "создал страницу + скрин + design-critique ВЫЗВАН → не нудж" 0
{ user_block; printf '\n'; browse; printf '\n'; claim 'Готово, обновил TODO.'; } > "$TMP"
massert "готово без визуала → не нудж" 0

rm -f "$TMP" .claude/visual-check-mobile-state.json .claude/visual-check-design-state.json
